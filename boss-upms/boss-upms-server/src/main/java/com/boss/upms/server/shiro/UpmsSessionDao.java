...
public class UpmsSessionDao extends EnterpriseCacheSessionDAO {
    ...
    // 全局会话redis的key
    private final static String BOSS_UPMS_SHIRO_SESSION_ID = "boss-upms-shiro-session-id";
    // 全局会话redis的id列表key
    private final static String BOSS_UPMS_SHIRO_SESSION_IDS = "boss-upms-shiro-session-ids";
    // token key
    ...
    protected Serializable doCreate(Session session) {
        ...
        RedisUtil.set((BOSS_UPMS_SHIRO_SESSION_ID + "_" + sessionId).getBytes(), sessionToByte(session), (int) session.getTimeout() / 1000);
        _log.debug("[UpmsSessionDao]创建session: sessionId={}", session.getId());
        // 维护会话id列表，提供会话分页管理
        Jedis jedis = RedisUtil.getJedis();
        jedis.lpush(BOSS_UPMS_SHIRO_SESSION_IDS, sessionId.toString());
        jedis.close();
        return sessionId;
    }
    ...
    protected Session doReadSession(Serializable sessionId) {
        ...
        if(session == null){
            byte[] bytes = RedisUtil.get((BOSS_UPMS_SHIRO_SESSION_ID + "_" + sessionId).getBytes());
    ...
    protected void doUpdate(Session session) {
        ...
        RedisUtil.set((BOSS_UPMS_SHIRO_SESSION_ID + "_" + session.getId()).getBytes(), sessionToByte(session), (int) session.getTimeout() / 1000);
    ...
    protected void doDelete(Session session) {
        ...
        RedisUtil.remove((BOSS_UPMS_SHIRO_SESSION_ID + "_" + serverSessionId).getBytes());
        _log.debug("[UpmsSessionDao]redis中删除session: sessionId={}", session.getId());

        // 维护会话id列表，提供会话分页管理
        jedis = RedisUtil.getJedis();
        jedis.lrem(BOSS_UPMS_SHIRO_SESSION_IDS, 1, serverSessionId);
        jedis.close();
    }
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        Jedis jedis = RedisUtil.getJedis();
        // 获取在线会话总数
        long total = jedis.llen(BOSS_UPMS_SHIRO_SESSION_IDS);
        // 获取当前页会话详情
        List<String> ids = jedis.lrange(BOSS_UPMS_SHIRO_SESSION_IDS, offset, (offset + limit - 1));
        List<Session> rows = new ArrayList<>();
        for (String id : ids) {
            byte[] bytes = RedisUtil.get((BOSS_UPMS_SHIRO_SESSION_ID + "_" + id).getBytes());
            if(null != bytes && bytes.length > 0){
    ...
    public int forceout(String ids) {
        ...
        for (String serverSessionId : sessionIds) {
            ...
            byte[] bytes = RedisUtil.get((BOSS_UPMS_SHIRO_SESSION_ID + "_" + serverSessionId).getBytes());
            if(null != bytes && bytes.length > 0){
